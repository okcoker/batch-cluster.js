import { EventEmitter, setImmediate } from '../deps.ts';
import { filterInPlace } from './Array.ts';
import {
    BatchClusterEmitter,
    BatchClusterEvents,
} from './BatchClusterEmitter.ts';
import {
    AllOpts,
    BatchClusterOptions,
    verifyOptions,
} from './BatchClusterOptions.ts';
import { BatchProcess, WhyNotReady } from './BatchProcess.ts';
...
export { BatchClusterOptions } from './BatchClusterOptions.ts';
export { Deferred } from './Deferred.ts';
export * from './Logger.ts';
export { SimpleParser } from './Parser.ts';
export { kill, pidExists, pids } from './Pids.ts';
...
export class BatchCluster {
    ...
    readonly #beforeExitListener: EventListenerOrEventListenerObject = () => {
        this.end(true);
    };
    readonly #exitListener: EventListenerOrEventListenerObject = () => {
        this.end(false);
    };
    ...
    async pids(): Promise<number[]> {
        ...
        for (const proc of this.#procs) {
            if (!proc.exited && (await proc.running())) {
                arr.push(proc.pid);
            }
        }
    ...
    vacuumProcs() {
        ...
        filterInPlace(this.#procs, (proc) => {
            ...
            const why = this.#procs.length > this.options.maxProcs
                ? 'tooMany'
            ...
            if (why) {
                ...
                this.#logger().debug(
                    `${proc.pid} ending proc in vacuum. reason: ${why}`,
                );
    ...
    #execNextTask(): boolean {
        ...
        if (!task) {
            this.emitter.emit(
                'internalError',
                new Error('unexpected null task'),
            );
            return false;
        }
    ...
    #maybeLaunchNewChild() {
        if (
            ...
            this.#lastSpawnedProcTime +
                        this.options.minDelayBetweenSpawnMillis <=
                Date.now()
        ) {
